...
async function renderMasters(){
  ...
  try {
    ...
    for(const entry of root.children){const m=masters.find(m=>m.id===entry.dataset.masterId);if(!m)continue;
      ...
      entry.removeButton.disabled=busy||m.common||m.projects.length>0;
    }
